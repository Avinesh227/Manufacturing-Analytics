...
with tab_id[2]:
    ...
    for cat in ["High", "Medium", "Low"]:
        radar_fig.add_trace(go.Scatterpolar(r=RADAR_DATA[cat], theta=RADAR_DATA['metric'], fill='toself', name=cat,
                                            line=dict(color=EFF_COLOR[cat])))
    radar_fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 100])), showlegend=True,
                            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="#c0d8f0",
                            height=400)
